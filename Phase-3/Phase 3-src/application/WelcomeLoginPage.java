    ...
    public void show( Stage primaryStage, User user) {
    	...
    	System.out.println(role);
    		...
    		for(int i = 0; i < role.length; i++) {
    			...
    			//Edited by Sofia: add a student-role specific button connecting the welcome page to the student question answer system
    			if(role[i].strip().equals("student")) {
    			    roleButton.setOnAction(b -> {
    			        Stage studentStage = new Stage(); // open a new window
    			        new StudentHomePage(QAdatabase, user).show(studentStage);
    			    ...
    			    roleButton.setOnAction(b -> {
    			        ...
    			        new ReviewerHomePage(QAdatabase, user).show(reviewerStage);
    			        System.out.println(user.getUserName() + "\n");
    			    });
    			

    			} else if (role[i].strip().equals("staff")) {
    				roleButton.setOnAction(b -> {
    					try {
    						new StaffHomePage().start(new Stage());
    					} catch (Exception ex) {
    						ex.printStackTrace();
    					}
    				});
